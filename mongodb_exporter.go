...
var (
	app            = kingpin.New(program, "")
	listenAddressF = app.Flag("web.listen-address", "Address to listen on for web interface and telemetry.").Default(":9216").String()
	metricsPathF   = app.Flag("web.telemetry-path", "Path under which to expose metrics.").Default("/metrics").String()

	collectDatabaseF   = app.Flag("collect.database", "Enable collection of Database metrics").Bool()
	collectCollectionF = app.Flag("collect.collection", "Enable collection of Collection metrics").Bool()
	collectTopF        = app.Flag("collect.topmetrics", "Enable collection of table top metrics").Bool()
	collectIndexUsageF = app.Flag("collect.indexusage", "Enable collection of per index usage stats").Bool()

	uriF = app.Flag("mongodb.uri", "MongoDB URI, format").
		PlaceHolder("[mongodb://][user:pass@]host1[:port1][,host2[:port2],...][/database][?options]").
		Default("mongodb://localhost:27017").
		Envar("MONGODB_URI").
		String()

	authDB   = app.Flag("mongodb.authentification-database", "Specifies the database in which the user is created").Default("").String()
	tlsF     = app.Flag("mongodb.tls", "Enable tls connection with mongo server").Bool()
	tlsCertF = app.Flag("mongodb.tls-cert", "Path to PEM file that contains the certificate (and optionally also the decrypted private key in PEM format).\n"+
		"    \tThis should include the whole certificate chain.\n"+
		"    \tIf provided: The connection will be opened via TLS to the MongoDB server.").Default("").String()
	tlsPrivateKeyF = app.Flag("mongodb.tls-private-key", "Path to PEM file that contains the decrypted private key (if not contained in mongodb.tls-cert file).").Default("").String()
	tlsCAF         = app.Flag("mongodb.tls-ca", "Path to PEM file that contains the CAs that are trusted for server connections.\n"+
	...
	tlsDisableHostnameValidationF = app.Flag("mongodb.tls-disable-hostname-validation", "Disable hostname validation for server connection.").Bool()
	maxConnectionsF               = app.Flag("mongodb.max-connections", "Max number of pooled connections to the database.").Default("1").Int()
	testF                         = app.Flag("test", "Check MongoDB connection, print buildInfo() information and exit.").Bool()

	socketTimeoutF = app.Flag("mongodb.socket-timeout", "Amount of time to wait for a non-responding socket to the database before it is forcefully closed.\n"+
		"    \tValid time units are 'ns', 'us' (or 'µs'), 'ms', 's', 'm', 'h'.").Default("3s").Duration()
	syncTimeoutF = app.Flag("mongodb.sync-timeout", "Amount of time an operation with this session will wait before returning an error in case\n"+
	...
	enabledGroupsFlag = app.Flag("groups.enabled", "Currently ignored").Default("").String()
)
