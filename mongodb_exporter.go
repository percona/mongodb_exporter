...
var (
	versionF       = kingpin.Flag("version", "Print version information and exit.").Short('v').Bool()
	...
	tlsCertF = kingpin.Flag("mongodb.tls-cert", "Path to PEM file that contains the certificate (and optionally also the decrypted private key in PEM format).\n"+
		...
		"    \tIf provided: The connection will be opened via TLS to the MongoDB server.").String()
	tlsPrivateKeyF = kingpin.Flag("mongodb.tls-private-key", "Path to PEM file that contains the decrypted private key (if not contained in mongodb.tls-cert file).").String()
	tlsCAF         = kingpin.Flag("mongodb.tls-ca", "Path to PEM file that contains the CAs that are trusted for server connections.\n"+
		...
		"    \tIf not provided: System default CAs are used.").String()
