...
type Optime struct {
	Ts bson.Timestamp `bson:"ts"` // The Timestamp of the last operation applied to this member of the replica set from the oplog.
...
type Members struct {
	...
	OptimeDate           bson.DateTime     `bson:"optimeDate"`           // The last entry from the oplog that this member applied.
	...
	ElectionTime         bson.Timestamp    `bson:"electionTime"`         // For the current primary, information regarding the election Timestamp from the operation log.
	ElectionDate         bson.DateTime     `bson:"electionDate"`         // For the current primary, an ISODate formatted date string that reflects the election date.
	LastHeartbeat        bson.DateTime     `bson:"lastHeartbeat"`        // Reflects the last time the server that processed the replSetGetStatus command received a response from a heartbeat that it sent to this member.
	LastHeartbeatRecv    bson.DateTime     `bson:"lastHeartbeatRecv"`    // Reflects the last time the server that processed the replSetGetStatus command received a heartbeat request from this member.
...
type ReplicaSetStatus struct {
	Date                    bson.DateTime `bson:"date"`                    // Current date
...
type ReplicasetConfig struct {
	...
	OK                  float64        `bson:"ok"`
	LastCommittedOpTime bson.Timestamp `bson:"lastCommittedOpTime"`
